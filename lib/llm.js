...
async function getOllama() {
  if (!_ollama) {
    ...
    const oneHourFetch = (input, init) => {
      const someInit = init || {};
      return undiciFetch(input, { ...someInit, dispatcher: new Agent({ headersTimeout: 3600000 }) });
    };
    _ollama = new Ollama({ host: process.env.OLLAMA_HOST || 'http://localhost:11434', fetch: oneHourFetch });
  }
  return _ollama;
}

async function ollamaChat({ system, messages, tools, model, thinking }) {
...
export async function chat(options) {
  ...
  return IS_OLLAMA ? ollamaChat(opts) : claudeChat(opts);
}
