...
// agentType is the authoritative label.
function labelForSubagent(metaPath, fallbackId) {
  try {
    const meta = JSON.parse(fs.readFileSync(metaPath, 'utf8'));
    const t = meta.agentType || '';
    if (t.startsWith('newsletter-')) {
      const phase = t.slice('newsletter-'.length);
...
// Build per-event records from a labeled set of scanned events.
function makeEvents(scanned, label) {
  const out = [];
  let total = 0;
...
export function computeCostEvents(connectedSessionId) {
  ...
  for (const file of subFiles) {
    ...
    const label    = labelForSubagent(metaPath, subId);
    if (label === null) continue;          // non-newsletter subagent — skip
    ...
    const ev        = makeEvents(scanned.events, label);
  ...
  return { events: allEvents, total: runningTotal };
}
