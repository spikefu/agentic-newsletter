...
async function launchChrome() {
  ...
  if (!launcher) {
    console.warn(
      `Chrome binary not found — start it manually with --remote-debugging-port=${CHROME_PORT}`
    )
    return
  }
  ...
  child.unref()
  console.log(
    `Launched Chrome on debug port ${CHROME_PORT} via ${launcher.cmd} (pid ${child.pid}, profile ${userDataDir})`
  )
}
...
async function tabsForRequest(req) {
  ...
  try {
    ...
    const target = all.find(
      (t) => t.type === 'page' && t.url?.includes(`nl-nonce=${nonce}`)
    )
    if (!target) return getChromeTabs()
    const candidates = filterPageTabs(all)
    const windowMap = await getWindowsForTargets([
      target.id,
    ...
    const tabs = candidates
      .filter((c) => windowMap.get(c.id) === windowId)
      .map((t) => ({ title: t.title || t.url, url: t.url }))
    return { tabs, error: null }
  } catch (e) {
    console.warn(
      'Window-scoped tab listing failed; falling back to unscoped:',
      e.message
    )
...
app.get('/api/prompts', (req, res) => {
  res.json({
    discovery: fs.existsSync(DISCOVERY_PROMPT)
      ? fs.readFileSync(DISCOVERY_PROMPT, 'utf8')
      : '',
    research: fs.existsSync(RESEARCH_PROMPT)
      ? fs.readFileSync(RESEARCH_PROMPT, 'utf8')
      : '',
  })
})
...
app.post('/api/prompts', (req, res) => {
  ...
  if (typeof discovery === 'string')
    fs.writeFileSync(DISCOVERY_PROMPT, discovery, 'utf8')
  if (typeof research === 'string')
    fs.writeFileSync(RESEARCH_PROMPT, research, 'utf8')
  res.json({ ok: true })
})
...
app.get('/api/settings', (req, res) => {
  res.json({
    ...loadSettings(),
    _meta: { provider: PROVIDER, model: MODEL, fastModel: FAST_MODEL },
  })
})
...
function _ccTouch() {
  _ccConn.lastActivityAt = Date.now()
}
...
function _ccPresence() {
  ...
  if (_ccConn.currentRunId) {
    ...
    if (now - lastSignal > CC_HEARTBEAT_TIMEOUT_MS) {
      ...
      return now - _ccConn.lastActivityAt < CC_RECONNECT_WINDOW_MS
        ? 'reconnecting'
        : 'not_connected'
    }
    return 'running'
  ...
  if (now - _ccConn.lastActivityAt < CC_RECONNECT_WINDOW_MS)
    return 'reconnecting'
...
app.post('/api/cc/connection', (req, res) => {
  ...
  if (previous && previous !== sessionId) {
    ...
    while (_ccAnswerWaitResolvers.length)
      _ccAnswerWaitResolvers.shift()({ takeover: true })
    _ccPendingWork = null
    _ccConn.currentRunId = null
  }
...
app.delete('/api/cc/connection', (req, res) => {
  ...
  _ccConn.currentRunId = null
  _ccPendingWork = null
  while (_ccWaitResolvers.length)
    _ccWaitResolvers.shift()({ disconnected: true })
  res.json({ ok: true })
})
...
app.post('/api/cc/run', async (req, res) => {
  ...
  if (presence === 'not_connected') {
    return res.status(503).json({
      ...
      message:
        "Claude Code isn't connected. From a CC session in this project, run `/newsletter` (or `newsletter wait` if you've set up the skill). Then click Generate again.",
    })
  }
  ...
  fs.mkdirSync(CACHE_DIR, { recursive: true })
  fs.writeFileSync(
    path.join(CACHE_DIR, 'run.json'),
    JSON.stringify({ runId, kind, mode, tabs }, null, 2)
  )
  ...
  const onConsumed = (work) =>
    settle(() => res.json({ ok: true, runId: work.runId }))
  const timer = setTimeout(() => {
    ...
    settle(() =>
      res.status(503).json({ error: 'cc_not_connected', held: true })
    )
...
app.post('/api/cc/event', (req, res) => {
  ...
  _ccTouch()
  if (type === 'run-started')
    _ccConn.currentRunId = data?.runId || _ccPendingWork?.runId || null
...
app.get('/api/cc/await-answer', (req, res) => {
  ...
  if (_ccPendingAnswer) {
    ...
    _ccPendingAnswer = null
    while (_ccAnswerWaitResolvers.length)
      _ccAnswerWaitResolvers.shift()(drained)
  }
  const timer = setTimeout(() => {
      ...
      try {
        res.write(
          `data: ${JSON.stringify({
            ...
            data: {
              message:
                'Claude Code disconnected mid-run. The cache is in a partial state; click ↺ Clear & Redo to start fresh.',
            },
          })}\n\n`
        )
      } catch {}
...
app.get('/api/status', (req, res) => {
  ...
  res.json({
    ...
    clusters: hasClusters
      ? JSON.parse(fs.readFileSync(CLUSTERS_CACHE, 'utf8'))
      : null,
    newsletter: hasNewsletter
      ? JSON.parse(fs.readFileSync(NEWSLETTER_CACHE, 'utf8'))
      : null,
...
app.post('/api/elicit', async (req, res) => {
  ...
  try {
    const result = await elicitContext(
      tabs,
      existingContext || '',
      bufferSend,
      resolveSettings(loadSettings().elicitor)
    )
...
app.get('/api/podcast-script', (req, res) => {
  if (!fs.existsSync(NEWSLETTER_SCRIPT))
    return res.status(404).json({ error: 'Not generated yet' })
  res.setHeader('Content-Type', 'text/plain; charset=utf-8')
  res.setHeader(
    'Content-Disposition',
    'attachment; filename="podcast-script.txt"'
  )
...
app.get('/api/podcast-script/generate', async (req, res) => {
  ...
  if (!fs.existsSync(NEWSLETTER_CACHE)) {
    send('error', {
      message: 'No newsletter cached yet — run the pipeline first.',
    })
    return res.end()
  }
  ...
  try {
    const { script } = await generatePodcastScript(
      newsletter,
      send,
      resolveSettings(loadSettings().podcast)
    )
...
app.post('/api/save-dist', (req, res) => {
  if (!fs.existsSync(NEWSLETTER_HTML)) {
    return res
      .status(400)
      .json({ error: 'No newsletter ready — run the pipeline first.' })
  }
...
app.get('/api/stream', async (req, res) => {
  ...
  try {
    ...
    if (!tabs.length) {
      send('error', {
        message:
          tabError || 'No pages open in Chrome. Open some tabs then click Run.',
      })
      return res.end()
    }
    ...
    if (!redo && fs.existsSync(CLUSTERS_CACHE)) {
      ...
      send('phase', {
        phase: 1,
        label: 'Discovery',
        message: `${clusters.length} clusters loaded from cache`,
      })
      send('clusters', { clusters })
      send('status', {
        message: 'Discovery cache hit — running research phase...',
      })
    } else {
      const result = await runDiscoveryAgent(
        discoveryPrompt,
        send,
        resolveSettings(agentSettings.discovery)
      )
    ...
    const { newsletter, cost: researchCost } = await runResearchAgent(
      clusters,
      researchPrompt,
      send,
      resolveSettings(agentSettings.research)
    )
    ...
    if (newsletter) {
      ...
      try {
        ...
        await printToPDF(
          `http://localhost:${PORT}/api/newsletter.html`,
          NEWSLETTER_PDF
        )
        hasPdf = true
      } catch (e) {
      ...
      send('output_ready', {
        html: '/api/newsletter.html',
        pdf: hasPdf ? '/api/newsletter.pdf' : null,
      })
    }
    ...
    send('done', {
      message: newsletter
        ? 'Newsletter complete!'
        : 'Pipeline finished (no newsletter produced).',
    })
  } catch (err) {
